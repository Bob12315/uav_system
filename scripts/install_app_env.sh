#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"

APP_ENV_NAME="${APP_ENV_NAME:-app}"
APP_PYTHON_VERSION="${APP_PYTHON_VERSION:-3.10}"

TUNA_MAIN_CHANNEL="https://mirrors.tuna.tsinghua.edu.cn/anaconda/pkgs/main"
TUNA_CONDA_FORGE_CHANNEL="https://mirrors.tuna.tsinghua.edu.cn/anaconda/cloud/conda-forge"

APT_PACKAGES=(
  git
  curl
  wget
  build-essential
  v4l-utils
  ffmpeg
  libgl1
  libglib2.0-0
  gstreamer1.0-tools
  gstreamer1.0-plugins-base
  gstreamer1.0-plugins-good
  gstreamer1.0-plugins-bad
  gstreamer1.0-plugins-ugly
  gstreamer1.0-libav
)

CONDA_PACKAGES=(
  pyyaml
  numpy
  pytest
)

PIP_PACKAGES=(
  pymavlink
)

TUNA_PYPI_INDEX="https://pypi.tuna.tsinghua.edu.cn/simple"

TOTAL_STEPS=5
CURRENT_STEP=0
CURRENT_STEP_NAME="initializing"

if [[ -t 1 && "${NO_COLOR:-}" == "" ]]; then
  RED="$(printf '\033[31m')"
  GREEN="$(printf '\033[32m')"
  YELLOW="$(printf '\033[33m')"
  BLUE="$(printf '\033[34m')"
  BOLD="$(printf '\033[1m')"
  RESET="$(printf '\033[0m')"
else
  RED=""
  GREEN=""
  YELLOW=""
  BLUE=""
  BOLD=""
  RESET=""
fi

die() {
  echo "${RED}${BOLD}ERROR:${RESET} $*" >&2
  exit 1
}

info() {
  echo
  echo "${BLUE}${BOLD}==>${RESET} $*"
}

warn() {
  echo "${YELLOW}${BOLD}WARN:${RESET} $*"
}

progress_bar() {
  local current="$1"
  local total="$2"
  local width=24
  local filled=$((current * width / total))
  local empty=$((width - filled))
  local bar=""

  printf -v bar "%*s" "${filled}" ""
  bar="${bar// /#}"
  printf "%s[%s" "${GREEN}" "${bar}"
  printf "%*s" "${empty}" "" | tr ' ' '-'
  printf "]%s %d/%d" "${RESET}" "${current}" "${total}"
}

step() {
  CURRENT_STEP=$((CURRENT_STEP + 1))
  CURRENT_STEP_NAME="$*"
  echo
  progress_bar "${CURRENT_STEP}" "${TOTAL_STEPS}"
  echo "  ${BOLD}$*${RESET}"
}

on_error() {
  local exit_code="$?"
  echo
  echo "${RED}${BOLD}INSTALL FAILED${RESET}"
  echo "${RED}Failed step:${RESET} ${CURRENT_STEP}/${TOTAL_STEPS} ${CURRENT_STEP_NAME}"
  echo "${RED}Exit code:${RESET} ${exit_code}"
  echo "Check the error output above, fix the issue, then run this script again."
  exit "${exit_code}"
}

on_success() {
  echo
  echo "${GREEN}${BOLD}INSTALL SUCCEEDED${RESET}"
}

require_command() {
  command -v "$1" >/dev/null 2>&1 || die "'$1' was not found."
}

check_linux_x86_64() {
  step "Checking system architecture"
  local kernel
  kernel="$(uname -s)"
  [[ "${kernel}" == "Linux" ]] || die "This x86 branch installer currently supports Linux only. Detected: ${kernel}"

  local arch
  arch="$(uname -m)"
  [[ "${arch}" == "x86_64" || "${arch}" == "amd64" ]] || die "This branch requires x86_64/amd64. Detected: ${arch}"
  echo "${GREEN}OK:${RESET} ${kernel} ${arch}"
}

check_conda_env() {
  step "Checking conda environment"
  require_command conda

  [[ -n "${CONDA_DEFAULT_ENV:-}" ]] || die "No active conda environment. Run: conda activate ${APP_ENV_NAME}"
  [[ "${CONDA_DEFAULT_ENV}" == "${APP_ENV_NAME}" ]] || die "Please activate conda env '${APP_ENV_NAME}' first. Current env: ${CONDA_DEFAULT_ENV}"

  local actual_python
  actual_python="$(python -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')"
  [[ "${actual_python}" == "${APP_PYTHON_VERSION}" ]] || die "Python ${APP_PYTHON_VERSION}.x is required for env '${APP_ENV_NAME}'. Current: ${actual_python}"
  echo "${GREEN}OK:${RESET} conda env '${CONDA_DEFAULT_ENV}', Python ${actual_python}.x"
}

install_system_deps() {
  step "Installing system packages"
  if ! command -v apt-get >/dev/null 2>&1; then
    warn "Skipping system package installation: apt-get was not found."
    echo "Install equivalent packages manually: ${APT_PACKAGES[*]}"
    return
  fi

  info "Installing Ubuntu/Debian system packages"
  if [[ "${EUID}" -eq 0 ]]; then
    apt-get update
    apt-get install -y "${APT_PACKAGES[@]}"
  else
    require_command sudo
    sudo apt-get update
    sudo apt-get install -y "${APT_PACKAGES[@]}"
  fi
}

install_conda_deps() {
  step "Installing app conda packages"
  info "Installing app conda packages from temporary Tsinghua mirrors"
  conda install -y \
    --override-channels \
    -c "${TUNA_CONDA_FORGE_CHANNEL}" \
    -c "${TUNA_MAIN_CHANNEL}" \
    "${CONDA_PACKAGES[@]}"

  info "Installing pip-only packages from temporary Tsinghua PyPI mirror"
  python -m pip install -i "${TUNA_PYPI_INDEX}" --upgrade pip
  python -m pip install -i "${TUNA_PYPI_INDEX}" "${PIP_PACKAGES[@]}"
}

verify_app_env() {
  step "Verifying app environment"
  info "Verifying app environment"
  python - <<'PY'
import numpy
import pymavlink
import pytest
import yaml

print("python import check passed")
print("numpy", numpy.__version__)
print("pytest", pytest.__version__)
PY

  cd "${REPO_ROOT}"
  python -m app.main --help >/dev/null
  python -m telemetry_link.main --help >/dev/null
  echo "App command checks passed"
}

main() {
  trap on_error ERR

  check_linux_x86_64
  check_conda_env
  install_system_deps
  install_conda_deps
  verify_app_env

  trap - ERR
  on_success

  info "Done"
  echo "App environment '${APP_ENV_NAME}' is ready."
  echo "YOLO environment installation is intentionally not handled by this script yet."
  echo "Try:"
  echo "  cd ${REPO_ROOT}"
  echo "  python -m app.main --no-yolo-udp --run-seconds 1 --send-commands false --no-ui --blackbox-enabled false"
}

main "$@"
